...
pgm = daft.PGM([3.5, 2], origin=[-0.5, -0.5])

# Observed nodes
pgm.add_node(daft.Node("L", "Lemons", 0.0, 0.5, observed=True, shape="ellipse", aspect=1.5, fontsize=8))
pgm.add_node(daft.Node("S", "Scurvy", 2.5, 0.5, observed=True, shape="ellipse", aspect=1.5, fontsize=8))


# Dashed, orange empirical arrow
pgm.add_edge("L", "S", plot_params={"ec": "darkorange", "lw": 3, "ls": "--"})

pgm.render()
fig = pgm.figure
# Title
fig.suptitle("Simplified Lind's Model (1747)", y=0.98)

legend_line = mlines.Line2D([], [], color="darkorange", linestyle="--", linewidth=2.5,label="Empirical discovery (1747)")
fig.legend(handles=[legend_line], loc="lower center", fontsize=10, frameon=False)
...
# Nodes (Acid in red to highlight focus of the wrong story)
pgm.add_node(daft.Node("A", "Acid", x_center, y_acid, observed=True,  plot_params={"fc": "lightcoral", "ec": "black", "lw": 1.5}, shape="ellipse", aspect=1.5, fontsize=7))
pgm.add_node(daft.Node("B", "Bacteria \nin Food", x_bacteria, y_bacteria, observed=True, shape="ellipse", aspect=1.5, fontsize=7))
pgm.add_node(daft.Node("S", "Scurvy", x_scurvy, y_scurvy, observed=True, shape="ellipse", aspect=1.5, fontsize=7))


# Add edges
# ❌ Incorrect causal chain (red, dotted)
pgm.add_edge("A", "B", plot_params={"ec": "crimson", "lw": 2.5, "ls": ":"})
pgm.add_edge("B", "S", plot_params={"ec": "crimson", "lw": 2.5, "ls": ":"})

# Render and decorate
pgm.render()
fig = pgm.figure
# Title
fig.suptitle("Updated Bacterial Theory (Late 19th Century)", y=1)
# Legend
wrong_chain = mlines.Line2D([], [], color="crimson", linestyle=":", linewidth=2.5,label="Incorrect causal chain")
fig.legend(handles=[wrong_chain], loc="lower center", ncol=2, frameon=False, fontsize=10)
...
pgm = daft.PGM([3.5, 4], origin=[-0.5, -0.5])
...
# Nodes: elliptical, monochrome, observed
node_style = {"shape": "ellipse", "aspect":1.5, "scale":1.3, "observed": True, "plot_params": {"fc": "white", "ec": "black", "lw": 1.2}}
pgm.add_node(daft.Node("V", "Vitamin C", x_center, y_acid,fontsize=8, **node_style))
pgm.add_node(daft.Node("C", "Collagen \nSynthesis", x_bacteria, y_bacteria, fontsize=8, **node_style))
pgm.add_node(daft.Node("S", "Scurvy", x_scurvy, y_scurvy, fontsize=8, **node_style))


# Add edges
# 1. Acid reduces bacteria (solid black)
pgm.add_edge("V", "C", plot_params={"ec": "green", "lw": 1})

# 2. Bacteria influence scurvy prevention (solid black)
pgm.add_edge("C", "S", plot_params={"ec": "green", "lw": 1})
...
fig = pgm.figure
# Title
fig.suptitle("Simplified modern understanding (1930s onwards)", y=1)

legend_line = mlines.Line2D([], [], color="green", linestyle="-", linewidth=2.5,label="Scientific analysis")
fig.legend(handles=[legend_line], loc="lower center", fontsize=10, frameon=False)
```
